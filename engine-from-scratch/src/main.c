...
void spawn_enemy(bool is_small, bool is_enraged, bool is_flipped)
{
	...
	f32 speed = SPEED_ENEMY_LARGE;
    vec2 size = { 20, 20 }, sprite_offset = {0, 10};
    ...
    if (other->collision_layer == COLLISION_LAYER_ENEMY) {
        for (usize i = 0; i < entity_count(); ++i) {
            ...
            if (entity->body_id == hit.other_id) {
				...
				bool is_small = entity->animation_id == anim_enemy_large_id || anim_enemy_small_enraged_id;
...
int main(int argc, char *argv[]) {
    ...
    while (!should_quit) {
		...
		//debug render physics bodies (bounding boxes)
        {
            for (usize i = 0; i < entity_count(); ++i) {
                Entity* entity = entity_get(i);
                Body* body = physics_body_get(entity->body_id);
